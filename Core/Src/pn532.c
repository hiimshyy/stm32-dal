...
{
	if (HAL_I2C_GetState(_config->hi2c) != HAL_I2C_STATE_READY) {
		...
		HAL_I2C_Master_Transmit(_config->hi2c, _config->i2c_addr, packet, 8 + cmdlen, 100);
	}
}
...
static bool isready(void)
{
	if (HAL_I2C_GetState(_config->hi2c) != HAL_I2C_STATE_READY) {
		uint8_t status;
		HAL_I2C_Master_Receive(_config->hi2c, _config->i2c_addr, &status, 1, 100);
		#ifdef PN532DEBUG
		if (_config->log) {
			_config->log("[PN532] - isready: %02X", status);
		}
		#endif
...
bool sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout)
{
	...
	if(!readack()){
		#ifdef PN532DEBUG
			if (_config->log) {
				_config->log("[PN532] - Timeout waiting for ACK");
			}
		#endif
		return false;
	}
...
bool setPassiveActivationRetries(uint8_t maxRetries)
{
	...
	pn532_packetbuffer[1] = 0x05; // Set the max number of retries
	pn532_packetbuffer[2] = 0xFF; // MxRtyATR (default = 0xFF)
	pn532_packetbuffer[3] = 0x01; // MxRtyPSL (default = 0x01)
	pn532_packetbuffer[4] = maxRetries; // Max retries

	#ifdef PN532DEBUG
		if (_config->log) {
			_config->log("[PN532] - SetPassiveActivationRetries: %02X", maxRetries);
		}
	#endif
...
bool readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout)
{
	...
	if (!sendCommandCheckAck(pn532_packetbuffer, 3, timeout)) {
		#ifdef PN532DEBUG
			if (_config->log) {
				_config->log("[PN532] - No card(s) read");
			}
		#endif
		return false;
	}
...
bool readDetectedPassiveTargetID(uint8_t *uid, uint8_t *uidLength)
{
	...
	#ifdef MIFAREDEBUG
		if (_config->log) {
			_config->log("[PN532] - ATQA: %02X\n", sens_res);
			_config->log("[PN532] - SAK: %02X\n", pn532_packetbuffer[11]);
		}
	#endif
...
uint8_t mifareclassic_AuthenticateBlock(uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
	...
	#ifdef MIFAREDEBUG
		_config->log("[PN532] - Trying to authenticate card "));
		PrintHex(_uid, _uidLen);
		_config->log("Using authentication KEY "));
	...
	if (pn532_packetbuffer[7] != 0x00) {
		#ifdef PN532DEBUG
			_config->log("Authentification failed: "));
...
{
	#ifdef MIFAREDEBUG
		_config->log("Trying to read 16 bytes from block: %d\n", blockNumber);
	#endif
	...
	if (!sendCommandCheckAck(pn532_packetbuffer, 4, PN532_TIMEOUT)) {
		#ifdef MIFAREDEBUG
		_config->log("Failed to receive ACK for read command\n"));
		#endif
		return 0;
	}
	...
	if (pn532_packetbuffer[7] != 0x00) {
		#ifdef MIFAREDEBUG
			_config->log("Unexpected response"));
...
{
	#ifdef MIFAREDEBUG
		_config->log("Trying to write 16 bytes to block %d\n", blockNumber);
	#endif
	...
	if (!sendCommandCheckAck(pn532_packetbuffer, 20, PN532_TIMEOUT)) {
		#ifdef MIFAREDEBUG
			_config->log("Failed to receive ACK for write command\n"));
		#endif
		return 0;
	}
...
{
	#ifdef MIFAREDEBUG
		_config->log("Trying to read 4 bytes from page %d\n", page);
	#endif
...
uint8_t mifareultralight_WritePage(uint8_t page, uint8_t *data)
{
	if (page >= 64) {
		#ifdef MIFAREDEBUG
			_config->log("Page value %d out of range\n", page);
		#endif
		return 0;
	}
	#ifdef MIFAREDEBUG
		_config->log("Trying to write 4 bytes to page %d\n", page);
	#endif
...
uint8_t ntag2xx_ReadPage(uint8_t page, uint8_t *buffer)
{
	...
	if (pn532_packetbuffer[7] == 0x00) {
		...
		#ifdef MIFAREDEBUG
			_config->log("Unexpected response reading block: ");
		    PrintHexChar(pn532_packetbuffer, 26);
		#endif
	    return 0;
	}
...
uint8_t ntag2xx_WritePage(uint8_t page, uint8_t *data)
{
	...
	#ifdef MIFAREDEBUG
		_config->log("Writing page %d: ", page);
	#endif
